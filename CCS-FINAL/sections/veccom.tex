...
Let $\hvc$ be a vector commitment scheme for domain $\moddomain$ and vector length $2^\tau$.
  $\hvc$ is individually correct, if for all security parameters $\secpar\in\NN$, vectors $\vec{m}\in\moddomain^{2^\tau}$, indices $1\leq t \leq 2^\tau$, parameters $\params \gets \setup(\secparam)$, commitments $\vec{c} \gets \commit(\params,\vec{m})$, and openings $\vec{d} \gets \open(\params,\vec{c},\vec{m},t)$ it holds that
  \[
...
parameters $\params \gets \setup(\secparam)$, commitments $\vec{c}^i\in\modcommit$, and decommitments $\vec{d}^i\in\modopening$ with $\iverify(\params,\vec{c}^i,t,\vec{d}^i) = \vec{m}^i$ such that $\vec{m}^i \neq \bot$ it holds that
  \begin{align*}
    &\Pr\mleft[
      ...
      \sverify\Bigl(\params,\sum_{i=1}^{\ell}w^i \vec{c}^i,t,\sum_{i=1}^{\ell}w^i \vec{d}^i\Bigr) = \sum_{i=1}^{\ell}w^i\vec{m}^i_{t}
    \mright] \\
    &\geq 1-\errorbound\enspace.
  \end{align*}
\end{definition}
%
% \gnote{There is actually no reason to restrict $\errorbound$ to $\NN$}
...
To commit to the correct domain $\ring_{q'}$, Squirrel adds some extra layers on the bottom of the tree.
...
\]
where $a'$ is the representative of $a$ in $\ring$ with the coefficients being in $\{-\frac{q-1}{2},\ldots,+\frac{q-1}{2}\}$.
\end{definition}
...
The map $\decompring$ is a one-sided inverse to $\projring$, meaning that 
\[
\projring(\decompring(a)) = a
\] for any $a\in\ring$.
...
Its commitments and openings are from $\modcommit = \ring_q$ and $\modopening = (\ring^\limbs)^{2\tau}\times (\ring^{\limbs'})^{\xi}$, where $\limbs = \normalceil{\log_{2\eta+1}q}, \limbs' = \normalceil{\log_{2\eta+1}q'}$.
\end{definition}

\begin{figure*}
\centering
\begin{pchstack}[boxed,aboveskip=0pt,belowskip=0pt]
\begin{pcvstack}
  ...
  \pcvspace
  %
  \procedure{$\commit(\params,\vec{m})$}{\\[-1.2em]
  ...
  \pchspace
  \begin{pcvstack}
  \procedure{$\open(\params,\vec{c},\vec{m},t)$}{\\[-1.2em]
    ...
    \end{pcvstack}
    \pchspace
  \procedure{$\verify(\params,\vec{c},t, \vec{d},\beta)$}{\\[-1.2em]
    \pcparse \vec{d} \pcas (\vec{p}_1,\dots,\vec{p}_\tau,\vec{s}_1,\dots,\vec{s}_\tau,\vec{u})\\
    \tilde t \coloneqq \binsca(t-1)\\
    \pcif \norm{\vec{u}} > \beta \pcor \vec{g}^\transpose \cdot \vec{u} \neq \projmod{q}(\vec{p}_\tau)\\[-1ex]
    \quad \pcreturn \bot\\
    %\vec{p}_0 \coloneqq \vec{c}\\
    \pcif \vec{c} \neq \vec{h}_{\tilde t_1}^\transpose\cdot \vec{p}_{1} + \vec{h}_{\tilde t_1 \xor 1}^\transpose \cdot \vec{s}_{1}\\[-1ex]
    \quad\pcreturn \bot\\
    \pcfor 2 \leq j \leq \tau\\
    \quad\pcif \projmod{q}(\vec{p}_{j-1}) \neq \vec{h}_{\tilde t_j}^\transpose\cdot \vec{p}_{j} + \vec{h}_{\tilde t_j \xor 1}^\transpose \cdot \vec{s}_{j}\\[-1ex]
    \quad\quad\pcreturn \bot\\
    \pcfor j \in \{1,\ldots,\tau\}\\
    \quad \pcif \lVert\vec{p}_{j}\rVert > \beta \pcor \lVert\vec{s}_{j}\rVert > \beta\\
    \quad \quad \pcreturn \bot \\[-1ex]
...
%     \quad \quad \pcreturn \bot\\
  ...
  \pchspace
  \begin{pcvstack}
    \procedure{$\iverify(\params,\vec{c},t, \vec{d})$}{\\[-1.2em]
      \pcreturn \verify(\params,\vec{c},t, \vec{d},\eta)
    }
    \pcvspace
    \procedure{$\sverify(\params,\vec{c},t, \vec{d})$}{\\[-1.2em]
      \pcreturn \verify(\params,\vec{c},t, \vec{d},\bagg)
    }
    \pcvspace
    \procedure{$\wverify(\params,\vec{c},t, \vec{d})$}{\\[-1.2em]
...
$\limbs = \normalceil{\log_{2\eta+1}q}$, $\limbs' = \normalceil{\log_{2\eta+1}q'}$.
Note that multiplication of $\ring_q$ with $\ring$ elements as in the computation of Ajtai hashes is understood to give an element in $\ring_q$, i.e.\ modular reduction is performed.
}
\label{fig:hvcinst}
\end{figure*}
...
\cameraready{%
\item The full version of this work~\cite{fullversion} contains a encoding scheme to transmit decommitments more efficiently. This requires some extra bounds checks on $\projring(\vec{p}_j)$ and $\projring(\vec{s}_j)$ inside $\verify$, which are not present here.
}
\end{enumerate}
\end{remark}
...
\bagg \geq \eta\sqrt{2\alpha_w\rho
  \bigl(
  \ln\tfrac{2n}{\errorbound} +  \ln(2\tau\limbs + \xi\limbs')
  \bigr)}\enspace.
\]
...
%  Let $\alpha$ be the smallest integer, such that $\binom{n}{\alpha}\cdot 2^\alpha \geq 2^\secpar$.
  If the problems 
  $
  \sis_{\ring,q,2\limbs q,4\bagg} \text{ and }\sis_{\ring,q,\xi\limbs' ,4\bagg}
  $ are hard,
  then \eprint{$\hvcplain$}\cameraready{$\hvccamera$} is an individually correct, a $(\rho,\tern_{\alpha_w},\errorbound)$-probabilistically homomorphic, robustly homomorphic, and position binding HVC for domain $\ring^{\xi}_{q'}$ and vector length $2^\tau$.
\end{theorem}
Due to space constraints the proof of \autoref{theo:veccom} is ommitted.
The proof follows the same ideas as the proof in \cite{CCS:FleSimZha22} and can be found in the full version~\cite{fullversion} of this work.
%\begin{proof}
%  The theorem follows from \autoref{lem:veccomcorrectness}, \autoref{lem:hvcprobhom}, \autoref{lem:hvcrobhom}, and \autoref{lem:hvcposbind} proven below.
%\end{proof}
...
%\end{lemma}
%\begin{proof}
%Let $\vec{m} \in (\ring_{q'}^{\xi})^{2^\tau}$, let $\vec{c} = \commit(\params,\vec{m})$, let $t\in[2^\tau]$, and let
%$(\vec{p}_1,\dots,\vec{p}_{\tau},\vec{s}_1, \dots, \vec{s}_{\tau},\vec{u})^\transpose = \open(\params,\vec{c},\vec{m},t)$. Let $\vec{p}_{0}, \tilde{t}$ be as in the definition of $\commit$.
%We first observe that for all $j\in\{1,\ldots,\tau\}$ it holds in $\ring_q$ that
%\begin{align*}
%  \projmod{q}(\vec{p}_{j-1})
%  ={}&\projmod{q}(\lbl_{\vec{g},\vec{h}_0,\vec{h}_1}(\vec{m},\tilde t_{< j}))\\
...
%      \end{aligned}
%    \right)\right)\\
%  ={}&\vec{h}_0^\transpose \cdot\lbl_{\vec{g},\vec{h}_0,\vec{h}_1}(\vec{m},\tilde t_{< j}\Vert 0)\\ + &\vec{h}_1^\transpose \cdot\lbl_{\vec{g},\vec{h}_0,\vec{h}_1}(\vec{m},\tilde t_{< j}\Vert 1)\\
...
%  ={}&\vec{h}_{\tilde t_{j}}^\transpose \cdot\vec{p}_{j} + \vec{h}_{\tilde t_{j}\xor 1}^\transpose \cdot\vec{s}_{j}.
%\end{align*}
%Observe that for $j=1$, this gives $\vec{c} = \vec{h}_{\tilde t_1}^\transpose \vec{p}_{1} + \vec{h}_{\tilde t_1 \xor 1}^\transpose  \vec{s}_{1}$ in $\ring_q$.
%Further it holds that
%\begin{align*}
%  \projmod{q}(\vec{p}_{\tau})
%  ={}&\projmod{q}(\lbl_{\vec{g},\vec{h}_0,\vec{h}_1}(\vec{m},\tilde t))\\
%  ={}&\projmod{q}(\decompmod{q}(\vec{g}^\transpose\cdot \decompmod{q'}(\vec{m}_t)))\\
%  ={}&\vec{g}^\transpose\cdot \decompmod{q'}(\vec{m}_t)
%  =\vec{g}^\transpose\cdot\vec{u}.
%\end{align*}
%%
...
%  \bagg \geq \eta\sqrt{2\alpha_w\rho
%  \bigl(
%  \ln\tfrac{2n}{\errorbound} +  \ln(2\tau\limbs  + \xi\limbs' \eprint{+ 2\tau})
%  \bigr)}\enspace,
%  \]
%  where $\limbs=\normalceil{\log_{2\eta+1}q}$ and $\limbs'=\lceil\log_{2\eta+1}q'\rceil$.
%  Let $\ring_q,\ring_{q'}$ be the polynomial rings $\defringq$ and $\defringqq$ respectively.
...
%\end{lemma}
%\begin{proof}
%Let $\params \gets \setup(\secparam)$, let $\vec{c}^i \in \ring_q^\limbs$, for $1 \leq t \leq 2^\tau$, let $\tilde{t} = \binsca(t-1)$,
%% $t\in\interval{2^\tau}$, $\tilde{t} = \binsca(t-1)$,
%and let $\vec{d}^i = (\vec{p}^i_1,\dots,\vec{p}^i_{\tau},\vec{s}^i_1, \dots, \vec{s}^i_{\tau},\vec{u})^\transpose \in \bigl(\ring^{\lceil\log_{2\eta+1} q\rceil}\bigr)^{2\tau} \times \ring^{\xi\lceil\log_{2\eta+1} q'\rceil}$
...
%\begin{align*}
%  \projmod{q}\Bigl(\sum_{i=1}^{\ell}w^i\cdot\vec{p}_{j-1}^i\Bigr) = &\sum_{i=1}^{\ell}w^i\cdot\projmod{q}(\vec{p}_{j-1}^i)\\
%  ={}&\sum_{i=1}^{\ell}w^i\cdot(\vec{h}_{\tilde t_j}^\transpose\cdot \vec{p}^i_{j} + \vec{h}_{\tilde t_j\xor 1}^\transpose \cdot \vec{s}^i_{j})\\
...
%and similarly
%\begin{align*}
%  \projmod{q}\Bigl(\sum_{i=1}^{\ell}w^i\cdot\vec{p}_{\tau}^i\Bigr) ={}&\sum_{i=1}^{\ell}w^i\cdot\projmod{q}(\vec{p}_{\tau}^i)\\
%  ={}&\sum_{i=1}^{\ell}w^i\cdot(\vec{g}^\transpose\cdot \vec{u}^i)
...
%\end{align*}
%due to \autoref{prop:projanddecomp} and the Definition of $\iverify$.
...
%\begin{align}
%  \lVert\vec{p}^i_{j}\rVert \leq \bagg\quad\text{and}\quad
%  \lVert\vec{s}^i_{j}\rVert \leq \bagg\label{eq:robhomnormcheck}\\
...
%  \begin{align*}
%    \lVert\vec{p}_j^0-\vec{p}_j^1\rVert \leq& \lVert{\vec{p}_j^0}\rVert + \lVert{\vec{p}_j^1}\rVert \leq 2\bagg\\
%    \lVert{\vec{s}_j^0-\vec{s}_j^1}\rVert \leq& \lVert{\vec{s}_j^0}\rVert + \lVert{\vec{s}_j^1}\rVert \leq 2\bagg
%  \end{align*}
%  and also that
%  \[
%      \lVert{\vec{u}^0-\vec{u}^1}\rVert \leq \lVert{\vec{u}^0}\rVert + \lVert{\vec{u}^1}\rVert \leq 2\bagg\enspace.
%  \]
%  \eprint{%
...
%    ={} &\projmod{q}(\vec{p}^0_{j-1})-\projmod{q}(\vec{p}^1_{j-1})\\
%    ={} &(\vec{h}_{\tilde t_j}^\transpose \vec{p}^0_{j} + \vec{h}_{\tilde t_j\xor 1}^\transpose  \vec{s}^0_{j})- (\vec{h}_{\tilde t_j}^\transpose \vec{p}^1_{j} + \vec{h}_{\tilde t_j\xor 1}^\transpose  \vec{s}^1_{j})\\
%    ={} &\vec{h}_{\tilde t_j}^\transpose (\vec{p}^0_{j} - \vec{p}^1_{j}) + \vec{h}_{\tilde t_j\xor 1}^\transpose  (\vec{s}^0_{j} - \vec{s}^1_{j})\enspace.
%  \end{align*}
%  and
%  \begin{align*}
%    \projmod{q}(\vec{p}^0_{\tau}-\vec{p}^1_{\tau})
%    ={} &\projmod{q}(\vec{p}^0_{\tau})-\projmod{q}(\vec{p}^1_{\tau})\\
%    ={} &(\vec{g}^\transpose \vec{u}^0 - \vec{g}^\transpose  \vec{u}^1)
%    =\vec{g}^\transpose (\vec{u}^0 - \vec{u}^1)\enspace.
%  \end{align*}
...
%  Let $n,q,q',\alpha_w,\rho,\eta,\tau,\xi,\bagg$ be positive integers and $0 < \errorbound \leq 1$, such that $n$ is a power of two, $q,q'$ are prime.
%  Let $\limbs \coloneqq \normalceil{\log_{2\eta+1}q}$ and $\limbs' \coloneqq \normalceil{\log_{2\eta+1}q'}$.
%  Let $\ring_q,\ring_{q'}$ be the polynomial rings $\defringq$ and $\defringqq$ respectively.
%  If the problems
%  $ 
%  \sis_{\ring,q,2\limbs,4\bagg} \text{ and } \sis_{\ring,q,\xi\limbs',4\bagg}
%  $ are hard, then \eprint{$\hvcplain$}\cameraready{$\hvccamera$} is position binding.
%\end{lemma}
%\begin{proof}
...
%    \leq{}& &&\negl\enspace,
%  \end{alignat*}
%  where we used the definition of $\wverify$ and the last inequality follows from the assumed hardness of the $\sis_{\ring,q,\xi\limbs',4\bagg}$ problem and the fact that all involved algorithms are PPT.
%  
%  We now analyze 
%  \[
%    \Pr[\vec{m}_0\neq \vec{m}_1 \land \bot\not\in\{\vec{m}_0,\vec{m}_1\} \land \vec{p}_\tau^0 \bmod q \neq \vec{p}_\tau^1 \bmod q]\enspace.
%  \]
%  We construct a PPT algorithm $\sisadv$ that solves the $\sis_{\ring,q,2 \limbs,4\bagg}$ problem as follows.
%  Upon input $\vec{a}=(a_1,\dots,a_{2 \limbs})^\transpose$,
%  $\sisadv$ sets 
%  $\vec{h}_0\coloneqq(a_1,\dots,a_{\limbs})^\transpose$ and 
%  $\vec{h}_1 \coloneqq (a_{\limbs+1},\dots,\allowbreak a_{2\limbs})^\transpose$
%  then samples $\vec{g} \gets \ring_q^{\xi\limbs'}$, sets $\params \coloneqq (\vec{g}, \vec{h}_0, \vec{h}_1)$, and runs 
...
%  Otherwise, parse $\vec{d}^i$ as $(\vec{p}^i_1,\dots,\allowbreak\vec{p}^i_{\tau},\vec{s}^i_1,\dots,\vec{s}^i_\tau,\vec{u}^i)$, set $\vec{p}^i_0 \coloneqq \decompmod{q}(\vec{c})$, $\tilde{t}\coloneqq \binsca(t-1)$.
%  
...
%  \[
%  \lVert{\vec{p}_{j^*}^0}\rVert \leq 2\bagg,\quad\!\! \lVert{\vec{s}_{j^*}^0}\rVert \leq 2\bagg, \quad\!\!\lVert{\vec{p}_{j^*}^1}\rVert \leq 2\bagg, \quad\!\! \lVert{\vec{s}_{j^*}^1}\rVert \leq 2\bagg.
%  \]
%  Therefore, the norm of $\vec{z}$ can be bounded as
%  \[
%  \lVert{\vec{z}}\rVert \leq \max\{\lVert{\vec{p}_{j^*}^0}\rVert,\lVert{\vec{s}_{j^*}^0}\rVert\}+\max\{\lVert{\vec{p}_{j^*}^1}\rVert,\lVert{\vec{s}_{j^*}^1}\rVert\} \leq 4\bagg\enspace.
%  \]
%  It remains to show that $\vec{z}\neq 0$.
%  Since $j^*$ is the \emph{largest} index such that 
%  $
%    \projmod{q}(\vec{p}_{j^*-1}^0) =  \projmod{q}(\vec{p}_{j^*-1}^1)
%  $,
%  it holds that
%  $
...
%  \begin{align*}
%  &\Pr[
%      \vec{a} \gets \ring_q^{2\limbs}; \vec{z}\gets\sisadv(\vec{a}) \colon \vec{z}\in\ball{4\bagg}{q}^{2\limbs}\setminus\{\vec{0}\} \land \vec{a}^\transpose\vec{z}\equiv 0
%    ]\\
%    ={}&
%    \Pr[\vec{m}_0\neq \vec{m}_1 \land \bot\not\in\{\vec{m}_0,\vec{m}_1\} \land \vec{p}_\tau^0 \bmod q\neq \vec{p}_\tau^1\bmod q] \\
%    \leq{}& \negl.
%  \end{align*}
...
%An opening consists of $\oplen = 2\tau\normalceil{\log_{2\eta+1}q} + \xi\normalceil{\log_{2\eta+1}q'}$ many elements from $\ring$.
...
% \begin{tabular}{c@{\hskip 1ex}l}
%  \toprule
%  Component& Size in Bits \\
%  \midrule
%  Commitment & $n\ceil{\log q}$\\
%  Individually Verifying Opening & $\bigl(\tau\limbs + \xi\limbs'\bigr)n\cdot \ceil{\log(2\eta+1)}$\\%\cline{2-3}
%  Strongly Verifying Opening & $\bigl(2\tau\limbs + \xi\limbs'\bigr)n\cdot \ceil{\log(2\bagg+1)}$\\
%                           \bottomrule
% \end{tabular}
% \medskip % Alternatively, put caption on top (some style guides suggest this)
% \caption{\cameraready{Bitlength of our HVC construction $\hvccamera$. We denote by $\limbs = \normalceil{\log_{2\eta+1}q}$ and $\limbs' = \normalceil{\log_{2\eta+1}q'}$ the number of limbs for the decompositions of $\ring_q$ resp.\ $\ring_{q'}$ elements. Note that in the individually verifying case, we do not include the $\vec{p}_j$ here, but only the sibling nodes $\vec{s}_j$.}}
