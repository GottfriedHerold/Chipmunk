...
%  Let $\alpha$ be the smallest integer, such that $\binom{n}{\alpha}\cdot 2^\alpha \geq 2^\secpar$.
  If the problems 
  \[
  \sis_{\ring,q,2\ceil{\log_{2\eta+1} q},4\bagg} \text{ and }\sis_{\ring,q,\xi\ceil{\log_{2\eta+1} q'} ,4\bagg}
  \] are hard,
...
\begin{align*}
  ...
  ={}&\projmod{q}\left(\lbl_{\vec{g},\vec{h}_0,\vec{h}_1}(\vec{m},\tilde t_{< j})\right)\\
...
By Equations \ref{eq:robhompathcheck} and \ref{eq:robhompaycheck} and the linearity of $\projmod{q}$ it follows that for all $j \in \interval{\tau}$, it holds in $\ring_q$ that
  \begin{align*}
    ...
    ={} &(\vec{h}_{\tilde t_j}^\transpose\cdot \vec{p}^0_{j} + \vec{h}_{\tilde t_j\xor 1}^\transpose \cdot \vec{s}^0_{j})- (\vec{h}_{\tilde t_j}^\transpose\cdot \vec{p}^1_{j} + \vec{h}_{\tilde t_j\xor 1}^\transpose \cdot \vec{s}^1_{j})\\
  ...
  \begin{align*}
    ...
    ={} &(\vec{g}^\transpose\cdot \vec{u}^0 - \vec{g}^\transpose \cdot \vec{u}^1)\\
...
\begin{lemma}\label{lem:hvcposbind}
  ...
  If the problems
  \[ 
  \sis_{\ring,q,2\normalceil{\log_{2\eta+1} q},4\bagg} \text{ and } \sis_{\ring,q,\xi\normalceil{\log_{2\eta+1} q'},4\bagg}
...
%  
  \begin{alignat*}{2}
    ...
    \leq{}& &&\negl\enspace,
  \end{alignat*}
  where we used the definition of $\wverify$ and the last inequality follows from the assumed hardness of the $\sis_{\ring,q,\xi\normalceil{\log_{2\eta+1} q'},4\bagg}$ problem and the fact that all involved algorithms are PPT.
