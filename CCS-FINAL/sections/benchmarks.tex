...
\begin{table}
  \centering
  \scalebox{.9}{
  \begin{tabular}{@{\makebox[2em][r]{\rownumber\space}} l@{\hspace{1em}}rl}
  \toprule
   \multicolumn{1}{@{\makebox[2em][r]{\#\space}} c}{Source}&\multicolumn{2}{c}{Constraint}\\
  \midrule
   \autoref{theo:veccom}&$\bagg \geq$&$ \eta\sqrt{2\alpha_w\rho(\ln \frac{2n}{\errorbound} + \ln(2\tau \kappa + \xi\kappa' \eprint{+ 2\tau}))}$\\
   \autoref{theo:veccom}&$\kappa=$&$\normalceil{\log_{2\eta+1}q}$\\
   \autoref{theo:veccom}&$\kappa'=$&$\normalceil{\log_{2\eta+1}q'}$\\
   \autoref{theo:kots}& $\beta_\sigma \geq$&$ 4\varphi\alpha_H\sqrt{\tfrac{1}{2}\alpha_w\rho \cdot \ln \frac{2n\otspkkeylen}{\errorbound} }$\\
   \autoref{theo:kots}&$\abs{\tern_{\alpha_H}} \geq$&$ 2^{2\secpar}$\\
   \autoref{theo:kots}& $\otspkkeylen\geq$&$((3\secpar+\delta)/n+\log_2q')\log^{-1}_2(\varphi+\tfrac{1}{2})$\\
   \autoref{theo:kots}& $\abs{\tern_{\alpha_H}} \leq$&$ 2^{2\secpar + \delta}$\\
   \autoref{theo:kots}&$q'>$&$ 16 \alpha_w \alpha_H\varphi$\\
   \autoref{def:actual_chipmunk}&$\xi=$& $2$\\
   \autoref{thm:mainconstruction}&$\chi \geq$ &$\secpar/\log(\tfrac{1}{2\errorbound})$\\
   \autoref{thm:mainconstruction}&$\abs{\tern_{\alpha_w}} \geq$&$2^\secpar$\\
  \bottomrule
  \end{tabular}
  }
...
\begin{table*}\centering
 ...
 \begin{tabular}{ll@{\hskip 4ex}l}
  ...
  \midrule
  \multirow{2}{*}{public parameters} & HVC & $n(\xi\limbs' + 2\limbs)\ceil{\log q}$\\\cline{2-3}
...
First, to ensure security of Chipmunk, the specific ring-SIS problems in \autoref{theo:veccom} need to be hard.
