...
\begin{definition}[Multi-User Existential Unforgeability under Rerandomized Keys]
  ...
  \begin{align*}
   & \Pr\mleft[
      ...
      (i^*,m^*,\sigma^*,w^*) \gets \adv^{\widetilde\sign(\cdot,\cdot)}(\params,\vec{\opk})\\
%      \sigma \gets \sign(\params,\osk,m);\\
%      (m^*,\sigma^*,w_0^*,w_1^*) \gets \adv(\sigma)\\
      \end{aligned}\colon \mathsf{E}(i^*, m^*, \sigma^*, \omega^*) = 1
    \mright] \\
    &\leq \negl,
  \end{align*}
  where $\mathsf{E}(i^*, m^*, \sigma^*, \omega^*) = 1$, if simultaneously it holds that $m^*\not\in Q_{i^*}$,  $\abs{Q_{i^*}}\leq 1$, $w^* \in W'$, $\wverify(\params,w^*\cdot\opk_{i^*},m^*,\sigma^*) = 1$ and $\wverify(\params,w^*\cdot\opk_{i^*},m^*,\sigma^*) = 1$. 
  The oracle $\widetilde\sign(\cdot,\cdot)$ is defined as $\widetilde\sign(i,m) := \sign(\osk_i,m)$, $\vec{\opk} = (\opk_1, \dots, \opk_{T})$, and $Q_i$ denotes the set of messages for which a signing query with index $i$ has been made.
