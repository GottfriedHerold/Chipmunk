...
\begin{definition}[Balanced $(2\eta+1)$-ary decomposition of $\ring_q$ elements]
    ...
    \begin{align*}
        &\decomp_q: \ring_q \to \ring^{\lceil\log_{2\eta+1} q\rceil},\\
        &\decomp_q(a) = \left(\sum_{i=1}^{n} a_{i,1} x^{i-1}, \dots, \sum_{i=1}^{n} a_{i,\lceil\log_{2\eta+1} q\rceil} x^{i-1} \right).
    \end{align*}
\end{definition}
...
\begin{proof}
    \begin{align*}
      ...
      ={}& \sum_{i=1}^{n} x^{i-1}\cdot a_i=a\tag*{}
    \end{align*}
\end{proof}
...
\begin{pcvstack}
  ...
  \pcvspace
  \procedure{$\commit(\params,\vec{m})$}{\\[-3.5ex]
    \vec{c} := \lbl(\vec{g},\vec{h}_0,\vec{h}_1,\vec{m},\epsilon)\\
    \pcreturn \vec{c}
  }
\end{pcvstack}
%\begin{pchstack}[center]
    \procedure{$\open(\params,\vec{c},\vec{m},t)$}{\\[-3.5ex]
      \tilde t := \binsca(t)\\
      \pcfor j \in [\tau]\\
  ...
  \pchspace
  \procedure{$\verify(\params,\vec{c},t, \vec{d},\beta)$}{\\[-3.5ex]
    \pcparse \vec{d} \pcas (\vec{p}_1,\dots,\vec{p}_\tau,\vec{s}_1,\dots,\vec{s}_\tau,\vec{u})\\
    \tilde t := \binsca(t)\\
    \vec{p}_0 := \vec{c}\\
    \pcfor j \in [\tau]\\
    \quad \pcif \Vert{\vec{p}_{j}}\Vert > \beta \pcor \Vert{\vec{s}_{j}}\Vert > \beta\\
...
%\end{pchstack}
  \pchspace
\begin{pcvstack}
    \procedure{$\iverify(\params,\vec{c},t, \vec{d})$}{\\[-3.5ex]
      \pcreturn \verify(\params,\vec{c},t, \vec{d},\eta)
    }
    \pcvspace
    \procedure{$\sverify(\params,\vec{c},t, \vec{d})$}{\\[-3.5ex]
      \pcreturn \verify(\params,\vec{c},t, \vec{d},\bagg)
    }
    \pcvspace
    \procedure{$\wverify(\params,\vec{c},t, \vec{d})$}{\\[-3.5ex]
...
%  Let $\alpha$ be the smallest integer, such that $\binom{n}{\alpha}\cdot 2^\alpha \geq 2^\secpar$.
  If the two ring-SIS problems $\sis_{\ring,q,2\lceil \log_{2\eta+1} q \rceil,4\bagg}$ and $\sis_{\ring,q,\xi\lceil \log_{2\eta+1} q' \rceil,4\bagg}$ are hard, then the construction from \autoref{fig:hvcinst} is an individually correct, $(\rho,\tern_\alpha,\varepsilon)$-probabilistically homomorphic, robustly homomorphic, and position binding HVC for domain $\ring^{\xi}_{q'}$ and vector length $2^\tau$.
\end{theorem}
\begin{proof}
...
By design the range of $\decomp_q$ and $\decomp_{q'}$ are sets of vectors of polynomials with coefficients in $\{-\eta,\dots,\eta\}$ and the norm of each $\vec{p}_j$ and $\vec{s}_j$ as well as $\vec{u}$ is at most $\eta$.
\end{proof}
...
\[
    \Pr\mleft[
      ...
      \lor &\bigl\Vert\sum_{i=1}^{\ell}w^i\cdot \vec{s}_j^i\bigr\Vert > \bagg\\
      \lor &\bigl\Vert\sum_{i=1}^{\ell}w^i\cdot\vec{u}^i\bigr\Vert > \bagg
      \end{aligned}
...
We will prove this lemma by leveraging that any pair of valid decommitments for different messages will lead to a collision somewhere in the generalized hash tree, which can be turned into a solution for one of the SIS instances. 
  ...
  \begin{align*}
    ...
    \leq{}&\Pr\mleft[\begin{aligned}
    &\proj_{q'}(\vec{u}^0) \neq \proj_{q'}(\vec{u}^1) \land \vec{g}^\intercal\cdot \vec{u}^0 = \vec{g}^\intercal\cdot \vec{u}^1\\
     &\land \norm{\vec{u}^0}\leq 2\bagg \land \norm{\vec{u}^1}\leq 2\bagg\end{aligned}\mright]\tag{Def. of $\wverify$}\\
  ...
  Upon input $\vec{a}=(a_1,\dots,a_{2\lceil\log_{2\eta+1} q\rceil})^\intercal$, $\bdv$ sets $\vec{h}_0:=(a_1,\dots,a_{\lceil\log_{2\eta+1} q\rceil})^\intercal$ and $\vec{h}_1 := (a_{\lceil\log_{2\eta+1} q\rceil+1},\dots,\allowbreak a_{2\lceil\log_{2\eta} q\rceil-1})^\intercal$, samples $\vec{g} \gets \ring_q^{\xi\lceil\log_{2\eta+1} q'\rceil}$ and runs $(\vec{c},t,\vec{d}^0,\vec{d}^1) \gets \adv((\vec{g},\vec{h}_0,\vec{h}_1))$.
  ...
  \negl \geq{}& \Pr\mleft[
  \begin{aligned}
      &\vec{a} \gets \ring_q^{2\lceil\log_{2\eta+1} q\rceil}\\ &\vec{z}\gets\bdv(\vec{a})
      \end{aligned}
       : \vec{z}\in\ball_{4\bagg}^{2\lceil\log q\rceil}\setminus\{\vec{0}\} \land \vec{a}^\intercal\vec{z}=0
    \mright]\\
    ={}&
    \Pr[\vec{m}_0\neq \vec{m}_1 \land \bot\not\in\{\vec{m}_0,\vec{m}_1\} \land \vec{p}_\tau^0 \neq \vec{p}_\tau^1]
  \end{align*}.
