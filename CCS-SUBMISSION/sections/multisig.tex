...
\begin{aligned}
  ...
  &\begin{pmatrix}\pubkeys,\\t,\\m,\\\aggsig\end{pmatrix}\gets\adv^{\sign(\params,\sk^*,\cdot,\cdot)}(\params,\pk^*)
  \end{aligned}
  \!\!\!: 
  \begin{aligned}
...
%    \pcparse \params \pcas (\hotsparams,\hvcparams)\\
    ...
    \pcreturn (\sk,\pk) := ((\osks,\opks),\vec{c})
  }
\end{pcvstack}
\pchspace
\begin{pcvstack}
  ...
  \procedure{$\aggregate(\params,\pubkeys,t,m,\signatures)$}{
    ...
    \pcdo[]\\
    \quad j:= j+1\\
    \quad (w^0,\dots,w^{\abs{\pubkeys}}) := H(t,m,\pubkeys,j)\\
    \quad \otsig := \sum_{i=1}^{\abs{\pubkeys}} w^i\cdot \otsig^i\\
    \quad \vec{d} := \sum_{i=1}^{\abs{\pubkeys}} w^i\cdot \vec{d}^i\\
    \pcwhile j < \chi \pcand \averify(\params,\pubkeys,t,m,(\otsig,\vec{d},j))=0\\
    \pcreturn \aggsig := (\otsig,\vec{d},j)
  }
  \end{pcvstack}
  \pchspace
  \begin{pcvstack}
  ...
  \procedure{$\averify(\params,\pubkeys,t,m,\aggsig)$}{
    (w^0,\dots,w^{\abs{\pubkeys}}) := H(t,m,\pubkeys,j)\\
    \vec{c} := \sum_{i=1}^{\abs{\pubkeys}} w^i\cdot \vec{c}_i\\
    \opk \gets \hvc.\sverify(\hvcparams,\vec{c},t,\vec{d})\\
...
\begin{proof}
    ...
    \begin{align*}
      ...
      =&\hots.\wverify(\hotsparams,\opk^0-\opk^1,m,\otsig^0-\otsig^1)=1 \tag*{}
    \end{align*}
\end{proof}
\fi
...
\begin{proof}
  The theorem follows immediately from \autoref{lem:msigindcorrect}, \autoref{lem:msigaggcorrect}, \autoref{lem:msigunf}.
\end{proof}
...
\begin{proof}
  ...
  \[\begin{aligned}
    ...
    \text{and} \quad &\vec{d} \gets \hvc.\open(\hvcparams,\vec{c},\opks,t).
    \end{aligned}
  \]
...
\begin{proof}
  ...
  Thus aggregation will succeed with overwhelming probability.
\end{proof}
...
\begin{proof}
  ...
  We stress that the proof of unforgeability in \cite{EPRINT:FleSimZha22} relies on a variant \cite{CCS:BelNev06} of the forking lemma \cite{EC:PoiSte96}, and therefore uses a rewinding strategy that does not apply to quantum algorithms.
  \iffalse
...
\begin{pchstack}[boxed,center]
  ...
  \procedure{$H(t,m,\pubkeys,j)$}{
    ...
    \vec{w} := (h_{i\rho+1},\dots,h_{i\rho+\ell})\\
    \pcfor j \in [\ell]\\
    \quad \pcif \vec{c}_j = \pk^*\\
    \quad\quad \vec{w} := \biggl(\begin{aligned}w^{0},\dots,w^{j-1},w^{\ell-1},\pclb w^{j+1},\dots,w^{\ell-2},w^{j}\end{aligned}\biggr)\\
    i := i+1\\
    \Hlist := \Hlist \cup ((t,m,\pubkeys,j),\vec{w})\\
    \pcreturn \vec{w}
  }
  \end{pcvstack}
  \pchspace
...
\begin{proof}
  ...
  After executing $\adv$, the algorithm $\bdv$ checks whether $\adv$ would have been successful according to \autoref{def:multisigunforge} and outputs $b=1$ iff $\adv$ was successful. Therefore, the claim follows. 
\end{proof}
...
Since $\hots$ is assumed to be multi-user existentially unforgeable under rerandomized keys, it thus follows that
  ...
  \]
  which implies that $\epsilon$ is negligible and concludes the proof.
  \fi
\end{proof}
