...
def print_sizes(tau, gamma, n, logq_hvc, logq_hots, log_beta_agg, log_beta_s):
    pk_size = ring_elem_size(n, logq_hvc)
    sig_size = ring_elem_size(n, logq_hvc) * tau + ring_elem_size(n, log_beta_s + 2) * gamma + 4 * ring_elem_size(n, logq_hots)
    agg_sig_size = ring_elem_size(n, log_beta_agg + 1) * logq_hvc * 2 * tau  \
                 + ring_elem_size(n, log_beta_agg + 1) * logq_hots * 4 \
                 + ring_elem_size(n, log_beta_sigma +1 ) * gamma
    tree_size =  ring_elem_size(n, logq_hvc) * 2^tau + ring_elem_size(n, logq_hots) * 2^tau
...
for tau in [21, 24, 26]:
    print("tau:", tau)
    print_sizes(tau, gamma, n, logq_hvc, logq_hots, log_beta_agg, log_beta_s)
