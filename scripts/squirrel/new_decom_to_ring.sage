...
def hash(param_a, input_x):
    '''
    param_a: a list of log(q) = 16 ring elements
    input_x: a list of log(q) = 16 ring elements of small norm
    return \\sum a_i * x_i
    '''
...
# =======================================
sample1 = vector([ZZ(i) for i in range(512)])
...
for _ in range (test_repeat):
    sample2 = vector([ZZ.random_element(0, q) for i in range(512)])
...
for _ in range(test_repeat):
    sample3 = vector([ZZ.random_element(0, q) for i in range(512)])
    sample3_dec = decompose(sample3)
    sample3_rec = project(sample3_dec)
    assert(sample3 == sample3_rec)

    sample4 = vector([ZZ.random_element(0, q) for i in range(512)])
    sample4_dec = decompose(sample4)
    sample4_rec = project(sample4_dec)
    assert(sample4 == sample4_rec)

    sum_sample_3_4 = vector([(sample3[i] + sample4[i])%q for i in range(512)])
...
param_a = []
for _ in range(16):
    a_i = vector([ZZ.random_element(0, q) for i in range(512)])
    x_i = vector([ZZ.random_element(0, 2) for i in range(512)])
    param_a.append(a_i)
    input_x.append(x_i)
...
# =======================================
# test for hash function with binary polynomial randomizer
# =======================================

## user 1's input
a = vector([ZZ.random_element(0, q) for i in range(512)])
a_decom = decompose(a)


## user 2's input
b = vector([ZZ.random_element(0, q) for i in range(512)])
b_decom = decompose(b)
...
##
## user 1's randomizer
w1 = vector([ZZ.random_element(0, 2) for i in range(512)])
## user 2's randomizer
w2 = vector([ZZ.random_element(0, 2) for i in range(512)])
...
## user 1's input
c = vector([ZZ.random_element(0, q) for i in range(512)])
c_decom = decompose(c)


## user 2's input
d = vector([ZZ.random_element(0, q) for i in range(512)])
d_decom = decompose(d)
...
w2 = vector([ZZ.random_element(0, 2) for i in range(512)])
## user 3's randomizer
w3 = vector([ZZ.random_element(0, 2) for i in range(512)])
## user 4's randomizer
w4 = vector([ZZ.random_element(0, 2) for i in range(512)])
