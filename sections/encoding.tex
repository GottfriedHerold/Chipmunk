...
So our task boils down to efficiently encode small elements (in $\norm{.}_{\infty}$-norm) from the lattice $\latmodular \coloneqq \{\vec{x}\in\ring^\limbs \mid \projring(\vec{x})\bmod q = 0\}$, where $\limbs = \ceiled{\log_{2\eta+1}q}$.
...
\begin{align*}
\latring    \coloneqq &\{\vec{v}\in\ring^\gamma \mid \projring(\vec{v}) = 0\}\\
\latmodular \coloneqq &\{\vec{v}\in\ring^\gamma \mid \projmod{q}(\vec{v}) = 0\}\\
\end{align*}
...
\[
\projring\colon\;\ring^\gamma\to\ring, \quad \projring(v_0,v_1,\ldots, v_{\gamma-1}) = \sum_{i=0}^{\gamma-1} (2\eta+1)^i \cdot v_i\enspace.
\]
We thus have $\projring(\vec{b}_0) = q$ and $\projring(\vec{b}_i) = 0$ for $1\leq i \leq \gamma-1$.
...
\begin{figure}[ht]
 \begin{pchstack}[center,boxed]
  ...
  \pchspace
   ...
   \pcreturn \decompring(h'') + \delta_v \in\ring^\gamma
   }
 \end{pchstack}
 \caption{%
    Algorithms for encoding and decoding and element $\vec{v}\in\ring^\gamma$ when we are given $h = \projmod{q}(\vec{v})$.
...
Let $q,\eta,\gamma$ be positive integers with $q$ prime and $\gamma = \ceiled{\log_{2\eta+1}q}$. Let $\params$ be some public parameters determining those integers.
...
\begin{enumerate}
 ...
 \item For any $\vec{v}\in\ring^\gamma$ and $h = \projmod{q}(\vec{v})$ and $\overline{\vec{v}} \gets \latencode(\params, \vec{v}, h)$ we have\\
 ...
 \item For any $\vec{v}\in\ring^\gamma$ and $h \coloneqq \projmod{q}(\vec{v})$, $(\delta_q,\alpha_1,\ldots,\alpha_{\gamma-1}) \gets \latencode(\params,\vec{v},h)$, we have
 \begin{align*}
...
First, note that $\projring(\vec{b}_i) = 0$ for all $1\leq i \leq \gamma-1$.
...
\[
 ...
 < \frac{\norm{\projring(\vec{v})}}{q} + \frac{1}{2}
\]
For the other bound, let us look at the individual components of $\vec{v}$ and $\decompring(\projring(\vec{v})) \in\ring^\gamma$.
For this, set $(w_0,\ldots, w_{\gamma-1})  \coloneqq \decompring(\projring(\vec{v}))$ and 
...
Let $n, q, q',\alpha_w,\rho, \eta, \tau, \xi, \bagg, \varepsilon$ be positive integers such that $n$ is a power of 2, $q,q'$ prime.
...
A commitment is a (non-short) single element from $\ring_q$. This means we can use $n\ceiled{\log q}$ bits to store it.\footnote{In principle, we could do $\ceiled{n\log q}$ by using some clever arithmetic encoding; hoever, for simplicity, we assume here that every coefficient is stored individually.}
In the variant without our elaborate encodings, an opening consists of $\oplen = 2\tau\ceiled{\log_{2\eta+1}q} + \xi\ceiled{\log_{2\eta+1}q'}$ many elements from $\ring$.
Each element is $\norm{.}_\infty$-bounded: for individually verifying openings, the bound is $\eta$, giving $n\oplen\ceiled{\log (2\eta+1)}$ bits. For strongly verifying opening, the bound is $\bagg$, giving $n\oplen\ceiled{\log (2\bagg+1)}$ bits.

For $\hvcencoded$, an opening consists of the same number of elements from $\ring$, but we have tighter size constraints for $\tau\ceiled{\log_{2\eta+1}q}$ of them. For the individually verifying case, those elements are actually 0. In the strongly verifying case, the (non-attained) bound is $\tfrac{\bagg}{2\eta}+\frac12$, giving $n\tau\ceiled{\log_{2\eta+1}q}\ceiled{\log(\frac{\bagg}{\eta}+\frac12)}$ many bits for the $\overline{\vec{p}}_i$'s.

\begin{table}\centering
 \begin{tabular}{cccc}
  & & & size in bits \\\toprule
  commitments & & & $n\ceiled{\log q}$\\
  \hline
  \multirow{4}{*}{opening}& \multirow{2}{*}{without encoding} &individually verifying & $\bigl(2\tau\gamma + \xi\gamma'\bigr)n\cdot \ceiled{\log(2\eta+1)}$\\\cline{3-4}
                          &                                   &strongly verifying & $\bigl(2\tau\gamma + \xi\gamma'\bigr)n\cdot \ceiled{\log(2\bagg+1)}$  \\\cline{2-4}
                          & \multirow{2}{*}{with encoding}    &individually verifying & $\bigl(\tau\gamma + \xi\gamma'\bigr)n\cdot \ceiled{\log(2\eta+1)}$\\\cline{3-4}
                          &                                   &strongly verifying & $\bigl(\tau\gamma + \xi\gamma'\bigr)n\cdot \ceiled{\log(2\eta+1)}+ \tau\gamma n\ceiled{\log (\frac{\bagg}{\eta}+\frac12)}$\\
 \end{tabular}
 \caption{bitlength of our HVC constructions. We denote by $\gamma = \ceiled{\log_{2\eta+1}q}$ and $\gamma' = \ceiled{\log_{2\eta+1}q'}$ the number of limbs for the decompositions of $\ring_q$ resp.\ $\ring_{q'}$ elements.}
