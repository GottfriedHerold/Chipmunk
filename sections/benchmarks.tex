...
\begin{table}
  ...
  \midrule
   ...
   \autoref{lem:keyhidden}& $\gamma\geq$&$((3\secpar+\delta)/n+\log_2q')\log^{-1}_2(\varphi+\tfrac{1}{2})$ \gnote{Fixed $q\rightarrow q'$ here. Please double-check.}\\
  ...
  %\bottomrule
  \end{tabular}
  \gnote{$\xi = 2\gamma$ is missing, from the connection of the HVC and KOTS. I suggest renaming $\gamma$, anyway.}
...
The aggregated one time signature, i.e., $\gamma$ polynomials in $\mathcal{R}$ with norm bound $\beta_\sigma < 2^{20}$, that constitutes $8$ kilobytes. \gnote{grammar}
