...
\gnote{Tables need to change to accomodate updated aggregate signature size. Possibly also need to update non-aggregate sizes and sigs because optimization gives different params}
...
For convenience, the results of running the script for a range of reasonable input parameters are shown in \autoref{tab:param} in Appendix~\ref{sec:concreteparams}.\gnote{The bound from \autoref{lem:hvcprobhom} has changed; may need to re-run.}
\begin{table}
  \centering
  ...
  \midrule
   ...
   \autoref{lem:kots_correct}& $\beta_\sigma \geq$&$ 4\varphi\alpha_H\sqrt{\tfrac{1}{2}\alpha_w\rho(\varepsilon+1+\log_2n\otspkkeylen)\cdot\ln2}$\\
   \autoref{lem:kots_sis}&$\abs{\tern_{\alpha_H}} \geq$&$ 2^{2\secpar}$\\
   \autoref{lem:keyhidden}& $\otspkkeylen\geq$&$((3\secpar+\delta)/n+\log_2q')\log^{-1}_2(\varphi+\tfrac{1}{2})$ \gnote{Fixed $q\rightarrow q'$ here. Please double-check.}\\
...
\begin{table}
 ...
 \begin{tabular}{cc@{\hskip 4ex}l}
  ...
  \midrule
  \multirow{2}{*}{public parameters} & HVC: Ajtai's hash functions & $n(\xi\limbs' + 2\limbs)\ceiled{\log q}$\\\cline{2-3}
...
%  \smallskip% Remove together with gnote
%  \gnote{$\xi$ may be replaced by new variable for the HOTS length due to ambiguity of $\gamma$.}
...
\mbox{}\gnote{Needs rewriting and updating the actual numbers once optimization was run}.
...
The aggregated one time signature, i.e., $\otspkkeylen$ polynomials \gnote{Shouldn't this be a singe poly?} in $\mathcal{R}$ with norm bound $\beta_\sigma < 2^{20}$, that constitutes $8$ kilobytes. \gnote{grammar}
