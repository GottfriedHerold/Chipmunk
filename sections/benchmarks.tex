...
To find such concrete parameters we used a script\footnote{\label{fn:github}\url{https://github.com/GottfriedHerold/Chipmunk}}, which enumerates possible parameters that satisfy the constraints, that lead to hard ring-SIS problems, and that allow for efficient NTT evaluations.
...
Our benchmarks show that Chipmunk's key generation time is improved by a factor of $9$ and that the size of the aggregate signatures is reduced by a factor of $4.8$.
\bgroup
...
In~\autoref{fig:keygen}, we also plotted the the key generation times and aggregate signature sizes of Chipmunk for a growing tree height $\tau$.
As one can see, both of these benchmarks scale linearly in the height of the tree as expected.
