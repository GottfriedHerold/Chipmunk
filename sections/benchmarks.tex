...
\begin{table}\centering
  \begin{tabular}{clccccc}
    ...
    \multirow{3}{*}{1024}                
                    ...
                    &Agg. Sig. size   & 682 KB      & 572 KB    & 92 KB     & 7.4$\times$ & 6.2$\times$ \\\midrule
    \multirow{3}{*}{8192}                
                    &Aggregation      & -           & 9.6 sec   & 4.6 sec   &     -       & 2.1$\times$ \\%\cline{2-7}
                    &Batch Verification    
                                      & 294 ms      & 53  ms    &  45 ms    & 6.5$\times$ & 1.2$\times$ \\%\cline{2-7}
                    &Agg. Sig. size   & 5.5 MB      & 762 KB    & 109 KB    & 50 $\times$ & 7  $\times$\\ \bottomrule
  \end{tabular}
...
\begin{table}[t]\centering
  \begin{tabular}{ccccccc}
      ...
      \multirow{2}{*}{21} & \multirow{2}{*}{9.1 sec}  & \multirow{2}{*}{0.40 ms}  & 1024    &   468 ms     &  8.6 ms       & 83 KB     \\%\cline{4-7}
                          &                           &                           & 8192    &   3.8 sec    &  42.6 ms      & 101 KB     \\\hline

      \multirow{2}{*}{23} & \multirow{2}{*}{37.4 sec} & \multirow{2}{*}{0.44 ms}  & 1024    &   516 ms     &  8.9 ms       & 90 KB     \\%\cline{4-7}
                          &                           &                           & 8192    &   4.1 sec    &  46 ms        & 109 KB     \\\hline

      \multirow{2}{*}{26} & \multirow{2}{*}{  4 min}  & \multirow{2}{*}{0.44 ms}  & 1024    &   630 ms     &  9.4 ms       & 100 KB     \\%\cline{4-7}
                          &                           &                           & 8192    &   4.6 sec    &  51 ms        & 121 KB     \\\bottomrule
...
An aggregated signature of size approx $92$ Kilobytes,
...
The aggregated path and its adjacent nodes belong to the homomorphic vector commitment, i.e. $2(\tau-1)\limbs$ polynomials in $\ring$ with an infinity norm bound $\bagg$.
The aggregated decomposed public keys for the one time signature scheme, i.e., $2\limbs'$ polynomials in $\ring$ with a same norm bound $\bagg$.
We use the encoding method to encode those $2(\tau-1)\limbs + 2\limbs' = 126$ ring elements. This result into a hint element of size $n\log(q)/8= 1.2$ Kilobytes;
and $132$ ring elements whose coefficients are bounded by $\beta_{\texttt{encode}} = \frac{\beta_{\texttt{agg}}}{2\eta} < 2^9$. 
The size of those ring elements is $(2(\tau-1)\limbs + 2\limbs') n (\log(\beta_{\texttt{encode}})+1)/8 = 78.8$ Kilobytes.
The total size of the encoded blob is approximately $1.2 + 78.8 = 80$ Kilobytes.

The second component is the aggregated one time signature, i.e., $\otspkkeylen$ polynomials in $\ring$ with norm bound $\beta_\sigma < 2^{20}$, that constitutes $12.8$ kilobytes. %\gnote{@Zhenfei/Mark:Grammar}
