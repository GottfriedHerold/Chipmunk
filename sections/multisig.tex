...
\begin{definition}[Synchronized Multi-Signatures]\label{def:multisig} A synchronized $\rho$-wise multi-signature scheme for $2^\tau$ time periods is defined by five PPT algorithms $\msig=(\setup, \kgen, \sign,\allowbreak \aggregate,\allowbreak \iverify, \averify)$.
\begin{description}
...
\begin{pcvstack}
  ...
  \procedure{$\averify(\params,\pubkeys,t,m,\aggsig)$}{
    (w^1,\dots,w^{\abs{\pubkeys}}) := H(t,m,\pubkeys,j)\\
...
\end{proof}
\fi
The following theorem now states the security of the construction of Chipmunk presented in~\autoref{fig:nidvconst}. \gnote{Change $q$ to $q'$ for notational consistency with the HOTS?}
