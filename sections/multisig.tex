...
\begin{claim}\label{claim:bisasgoodasa}
  ...
  \omega) \gets \bdv^{\sign(\params,)}(x,h_1,\dots,h_{p})\end{aligned} : b = 1\right] = \errorbound(\secpar).\]
\end{claim}
\begin{proof}
...
\end{proof}
  ...
  \begin{equation}
    \errorbound \leq \frac{p}{\abs{\tern_\alpha}} + \sqrt{p\cdot\Pr\left[\begin{aligned}((\params,\pk^*),(\params,\sk^*)) \gets \IG(\secparam);\\ (b,\omega^0,\omega^1)\gets \fork^{\sign(\params,\sk^*,\cdot,\cdot)}_\bdv(\params,\pk^*)\end{aligned} : b=1 \right]}.\label{eq:forkingresult}
  \end{equation}
...
Since $\hots$ is assumed to be multi-user existentially unforgeable under rerandomized keys, it thus follows that
  ...
  \[
    \errorbound \leq \frac{p}{\abs{\tern_\alpha}} + \sqrt{p\cdot\negl}
  \]
  which implies that $\errorbound$ is negligible and concludes the proof.\qed
  \fi
