...
\nnote{conditions from Lemmas + condition on $\beta_\sigma$ based on $\varepsilon$}
Let $H : \bin^* \to \tern_{\alpha_H}$ be a hash function.
%$\binom{n}{\alpha_w}\cdot 2^{\alpha_w} \geq 2^\secpar$
Let $W' = \{w_0-w_1\mid w_0,w_1\in\tern_{\alpha_w} \land w_0\neq w_1\}$.
If the \nnote{parameters} $\sis_{\ring,q,\gamma,2\beta_\sigma+4\alpha_w\phi\alpha_H}$ problem is hard and $H$ is collision resistant, then the construction from~\autoref{fig:otsconstruction} is a $(\rho,\tern_{\alpha_w},\varepsilon)$-homomorphically correct and robustly homomorphic KOTS that is $W'$-multi-user existentially unforgeable under rerandomized keys.
\end{theorem}
\begin{proof}
