...
Set $\gamma \coloneqq \ceiled{\log_{2\eta+1}q}$
We denote by
\[
 \projmod{q}\colon\;\ring^\gamma \to \ring_q,\quad \projmod{q}(\vec{a}) \coloneqq \projring(\vec{a})\bmod q\in\ring_q
\]
and by
\[
 \decompmod{q}\colon\;\ring_q \to \ring^\gamma,\quad \decompmod{q}(a) \coloneqq \decompring(a'),\enspace,
\]
...
The decomposed elements are always from $\ring^\gamma$.
...
\begin{pchstack}[center]
  \procedure{$\setup(\secparam)$}{
    \vec{g} \gets \ring_{q}^{\xi\lceil\log_{2\eta+1} q'\rceil}\\ % NOTE: \ring_q rather than \ring_{q'} is correct here.
