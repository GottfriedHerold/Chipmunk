...
\[
\hashajtai(\vec{c}_1,\vec{c}_2) \coloneqq \vec{a}_1^\intercal \vec{c}_1 + \vec{a}_2^\intercal \vec{c}_2 \bmod q
\]
...
\[
\hashajtai(\vec{c}_1,\vec{c}_2) = \proj(\vec{p}) \mod q.
\]
...
In lattice terms, this means we need to solve some close(est) vector problem for the kernel of $\proj_q$.
The main observation is that this construction works for any $\proj_q$ for which we can find short preimages:
the homomorphic properties of the HVC are due to the fact that the equation above\footnote{The point here is that what matters is that the verification equation is linear, not the map that finds the small preimage and is used to construct the Merkle tree. Indeed, the latter map is not linear.} is $\ring_q$-linear and the sum of small elements stays small.
...
\begin{definition}[Projection onto $\ring$ elements]\label{def:proj}
  ...
  \[
    \proj_q : \ZZ^{\lceil\log_{2\eta+1} q\rceil} \to \ZZ,\quad \proj_q(\vec{b}) = \smashoperator{\sum_{j=0}^{\lceil\log_{2\eta+1} q\rceil-1}} b_j\cdot(2\eta+1)^{j}.
  \]
  We can extend this to a map 
  \[
    \proj_q : \ring^{\lceil\log_{2\eta+1} q\rceil} \to \ring,\quad \proj_q(\vec{b}) = \smashoperator{\sum_{j=0}^{\lceil\log_{2\eta+1} q\rceil-1}} b_j\cdot(2\eta+1)^{j}.
  \]
\end{definition}
Note that $q$ only affects the lenght of the domain and is solely included for notational consistency with Squirrel. \gnote{Actually, parameterizing by $\eta$ and $\limbs$ and changing $2\eta+1$ to $\eta$ would make much more sense}
...
\begin{definition}[Balanced $(2\eta+1)$-ary decomposition of $\ring$ elements]
    ...
    Matching the notation from \autoref{def:proj}, by $\proj_q$ we denote this decomposition map with number of limbs $\limbs = \lceil \log_{2\eta+1} q \rceil$, giving a map
    \[
     \decomp_q \colon\, \ring \to \ring^{\lceil\log_{2\eta+1} q\rceil}\quad a \mapsto (a_0,\ldots,a_{\limbs-1})
    \]
    ...
    \[
     \decomp_q \colon\, \ring_q \to \ring^{\lceil\log_{2\eta+1} q\rceil}\quad a \mapsto (a_0,\ldots,a_{\limbs-1})
    \]
...
\begin{pchstack}[center]
  ...
  \procedure{$\commit(\params,\vec{m})$}{
    \vec{p}_0 := \lbl(\vec{g},\vec{h}_0,\vec{h}_1,\vec{m},\epsilon)\\
    \vec{c} := \proj_q(\vec{p}_0)\bmod q \\
...
\begin{pchstack}[center]
  ...
  \procedure{$\verify(\params,\vec{c},t, \vec{d},\beta)$}{
    ...
    \tilde t := \binsca(t)\\
    \pcif \norm{\vec{u}} > \beta \pcor \vec{g}^\intercal \cdot \vec{u} \neq \proj_q(\vec{p}_\tau) \bmod q\\
    \quad \pcreturn \bot\\
    %\vec{p}_0 := \vec{c}\\
    \pcfor j \in\{\tau,\tau-1,\ldots, 2\}\\
    \quad\pcif \proj_q(\vec{p}_{j-1}) \bmod q \neq \vec{h}_{\tilde t_j}^\intercal\cdot \vec{p}_{j} + \vec{h}_{\tilde t_j \xor 1}^\intercal \cdot \vec{s}_{j}\\
    \quad\quad\pcreturn \bot\\
    \pcif \vec{c} \neq \vec{h}_{\tilde t_1}^\intercal\cdot \vec{p}_{1} + \vec{h}_{\tilde t_1 \xor 1}^\intercal \cdot \vec{s}_{1}\\
    \quad\pcreturn \bot\\
    \pcfor j \in \{1,\ldots,\tau\}\\
    \quad \pcif \norm{\vec{p}_{j}} > \beta \pcor \norm{\vec{s}_{j}} > \beta\\
    \quad \quad \pcreturn \bot \\
    \quad \pcif \norm{\proj_q(\vec{p}_j)} > \tfrac{q\bagg}{2\eta} \pcor \norm{\proj_q(\vec{s}_j)} > \tfrac{q\bagg}{2\eta}\\
    \quad \quad \pcreturn \bot \\
...
\end{pchstack}
\end{pcvstack}
\caption{The construction of a homomorphic vector commitment for message space $\moddomain = \ring^\xi_{q'}$ based on a labeled binary tree.}
\label{fig:hvcinst}
\end{figure}
...
Chipmunk's homomorphic vector commitment actually employs a space-efficient non-trivial way to encode and decode (verifying) decommitments $\vec{d} = (\vec{p}_1,\ldots\,\vec{p}_\tau,\vec{s}_1,\ldots,\vec{s}_\tau,\vec{u})$.
This is, strictly speaking, part of the opening and verification algorithms.
For clarity, we describe this encoding and decoding separately in \autoref{TODO}.

If we set $\vec{p}_0$ as in the definition of $\commit$, the condition $\vec{c} = \vec{h}_{\tilde t_1}^\intercal\cdot \vec{p}_{1} + \vec{h}_{\tilde t_1 \xor 1}^\intercal \cdot \vec{s}_{1}$ is actually equivalent to
\[
\proj_q(\vec{p}_0 \equiv \vec{h}_{\tilde t_1}^\intercal\cdot \vec{p}_{1} + \vec{h}_{\tilde t_1 \xor 1}^\intercal \cdot \vec{s}_{1}.
\]
Hence, we may treat this condition as the special case $j=1$ of the condition $\proj_q(\vec{p}_{j-1}) \bmod q = \vec{h}_{\tilde t_j}^\intercal\cdot \vec{p}_{j} + \vec{h}_{\tilde t_j \xor 1}^\intercal \cdot \vec{s}_{j}$, which simplifies the analysis.

Let $\limbs := \lceil \log_{2\eta+1} q\rceil$
The inequalitity checks in the definition of $\verify$ all compare elements from $\ring_q$ and are to be taken modulo $q$. By contrast, the norm-bounds are to be taken in $\ring$.
For the individual verification, the condition that $\norm{\proj_q(\vec{p}_j)} \leq \tfrac{q\bagg}{2\eta}$ boils down to $\norm{\proj_q(\vec{p}_j)} \leq \tfrac{q}{2}$.
...
This may be possible because $q \neq (2\eta+1)^\limbs$. Furthermore, note that, particularly if $q$ is significantly smaller than $(2\eta+1)^\limbs$, adding this condition actually gives a stronger shortness bound for the most significant limbs of the decomposition.
These bounds are actually not required for our security proofs and not present in Squirrel, but they significantly help to make our encoding of openings more efficient and easier to analyze.
\end{remark}
...
\end{lemma}
\begin{proof}
Let $\vec{m} \in (\ring_{q'}^{\xi})^{2^\tau}$, $\vec{c} = \commit(\params,\vec{m})$, $t\in[2^\tau]$, $(\vec{p}_1,\dots,\vec{p}_{\tau},\vec{s}_1, \dots, \vec{s}_{\tau},\vec{u})^\intercal = \open(\params,\vec{c},\vec{m},t)$. Let $\vec{p}_{0}$ be as in the definition of $\commit$.
...
\begin{align*}
  ...
  \equiv{}&\proj_q\left(\decomp_q\left(\begin{aligned}
  ...
  \equiv{}&\vec{h}_0^\intercal \cdot\lbl(\vec{g},\vec{h}_0,\vec{h}_1,\vec{m},\tilde t_{< j}\Vert 0) + \vec{h}_1^\intercal \cdot\lbl(\vec{g},\vec{h}_0,\vec{h}_1,\vec{m},\tilde t_{< j}\Vert 1)\tag{\autoref{prop:projanddecomp}}\\
  \equiv{}&\vec{h}_{\tilde t_{j}}^\intercal \cdot\lbl(\vec{g},\vec{h}_0,\vec{h}_1,\vec{m},\tilde t_{< j}\Vert \tilde t_{j}) + \vec{h}_{\tilde t_{j}\xor 1}^\intercal \cdot\lbl(\vec{g},\vec{h}_0,\vec{h}_1,\vec{m},\tilde t_{< j}\Vert (\tilde t_{j}\xor 1))\\
  \equiv{}&\vec{h}_{\tilde t_{j}}^\intercal \cdot\vec{p}_{j} + \vec{h}_{\tilde t_{j}\xor 1}^\intercal \cdot\vec{s}_{j}\tag{Def. of $\open$}.
\end{align*}
Observe that for $j=1$, this gives $\vec{c} = \vec{h}_{\tilde t_1}^\intercal\cdot \vec{p}_{1} + \vec{h}_{\tilde t_1 \xor 1}^\intercal \cdot \vec{s}_{1}$ as elements in $\ring_q$.
...
\begin{align*}
  ...
  ={}&\vec{g}^\intercal\cdot \decomp_{q'}(\vec{m}_t)\tag{\autoref{prop:projanddecomp}}\\
...
For every $j\in[\tau]$, $\vec{p}_j$ and $\vec{s}_j$ are outputs of the $\lbl$ function and thus, by definition of $\lbl$, decompositions of elements from $\ring_q$.
...
It also implies that applying $\proj_q$ to $\vec{p}_j$ or $\vec{s}_j$ gives back the representive with coefficients in $\{-\tfrac{q-1}{2},\ldots,\tfrac{q-1}{2}\}$ that was decomposed.
...
\end{lemma}
\begin{proof}
Let $\params \gets \setup(\secparam)$, $\vec{c}^i \in \ring_q^{\lceil\log_{2\eta+1} q\rceil}$, $0 \leq t < 2^\tau$, $\vec{d}^i = (\vec{p}^i_1,\dots,\vec{p}^i_{\tau},\vec{s}^i_1, \dots, \vec{s}^i_{\tau},\vec{u})^\intercal\in (\ring^{\lceil\log_{2\eta+1} q\rceil})^{2\tau} \times \ring^{\xi\lceil\log_{2\eta+1} q'\rceil}$ with $\iverify(\params,\vec{c}^i,t,\vec{d}^i) = \vec{m}_t^i \neq \bot$
...
\begin{align*}
  \proj_q\Bigl(\sum_{i=1}^{\ell}w^i\cdot\vec{p}_{j-1}^i\Bigr) ={}&\sum_{i=1}^{\ell}w^i\cdot\proj_q(\vec{p}_{j-1}^i)\tag{\autoref{prop:projanddecomp}}\\
  \equiv{}&\sum_{i=1}^{\ell}w^i\cdot(\vec{h}_{\tilde t_j}^\intercal\cdot \vec{p}^i_{j} + \vec{h}_{\tilde t_j\xor 1}^\intercal \cdot \vec{s}^i_{j})\tag{Def. of $\iverify$}\\
  \equiv{}&\sum_{i=1}^{\ell}\vec{h}_{\tilde t_j}^\intercal\cdot w^i \vec{p}^i_{j} + \vec{h}_{\tilde t_j\xor 1}^\intercal\cdot w^i  \vec{s}^i_{j}\\  
  \equiv{}& \vec{h}_{\tilde t_j}^\intercal\cdot \Bigl(\sum_{i=1}^{\ell}w^i\cdot\vec{p}^i_{j}\Bigr) + \vec{h}_{\tilde t_j \xor 1}^\intercal \cdot \Bigl(\sum_{i=1}^{\ell}w^i\cdot\vec{s}^i_{j}\Bigr).
\end{align*}
and similarly
\begin{align*}
  ...
  \equiv{}&\sum_{i=1}^{\ell}w^i\cdot(\vec{g}^\intercal\cdot \vec{u}^i)\tag{Def. of $\iverify$}\\
  \equiv{}&\vec{g}^\intercal\cdot\sum_{i=1}^{\ell}w^i\vec{u}^i 
\end{align*}
and similarly that 
\begin{align*}
  \sum_{i=1}^{\ell}w^i\cdot \vec{c}^i \equiv{}&\sum_{i=1}^{\ell}w^i\cdot \vec{h}_{\tilde t_1}^\intercal\cdot \vec{p}_{1}^i + \vec{h}_{\tilde t_1 \xor 1}^\intercal \cdot \vec{s}_{1}^i\\
  \equiv{}&\vec{h}_{\tilde t_1}^\intercal\cdot \sum_{i=1}^{\ell}w^i\cdot \vec{p}_{1}^i + \vec{h}_{\tilde t_1 \xor 1}^\intercal \cdot \sum_{i=1}^{\ell}w^i\cdot \vec{s}_{1}^i
\end{align*}
...
\begin{align*}
    \Pr\Bigl[
      ...
      \exists j\in[\tau]\ldotp&
      \bigl\Vert\sum_{i=1}^{\ell}w^i\cdot \vec{p}_j^i\bigr\Vert > \bagg 
      \lor 
      \bigl\Vert\sum_{i=1}^{\ell}w^i\cdot \vec{s}_j^i\bigr\Vert
      \lor
...
By a union bound it is thus sufficient to show that each individual coefficient violates the bound with probability at most $2^{-\varepsilon}/(n(2\tau\lceil\log_{2\eta+1} q\rceil + \xi\lceil\log_{2\eta+1} q'\rceil)+2\tau)$.
  ...
  \begin{align*}
    ...
    ={}&\sum_{i=1}^{\ell}w^i \cdot \proj_{q'}(\vec{u}^i)\tag{\autoref{prop:projanddecomp}}\\
...
\begin{align*}
  ...
  =&\proj_{q'}(\vec{u}^0)-\proj_{q'}(\vec{u}^1)\tag{\autoref{prop:projanddecomp}}\\
...
We will prove this lemma by leveraging that any pair of valid decommitments for different messages will lead to a collision somewhere in the generalized hash tree, which can be turned into a solution for one of the SIS instances. 
  ...
  Otherwise, parse $\vec{d}^i$ as $(\vec{p}^i_1,\dots,\vec{p}^i_{\tau},\vec{s}^i_1,\dots,\vec{s}^i_\tau,\vec{u}^i)$, set $\vec{p}^i_0 := \decomp_q(\vec{c})$.
  
  Let $j^*\in [\tau+1]$ be the \emph{largest} index, such that $\proj_q(\vec{p}^0_{j^*-1})\not\equiv\proj_q(\vec{p}^1_{j^*-1})$.
