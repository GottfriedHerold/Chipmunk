...
%  Let $\alpha$ be the smallest integer, such that $\binom{n}{\alpha}\cdot 2^\alpha \geq 2^\secpar$.
  If the $\sis_{\ring,q,2\lceil \log_{2\eta+1} q \rceil,2\bagg}$ problem and the $\sis_{\ring,q,\xi\lceil \log_{2\eta+1} q' \rceil,2\bagg}$ problem are hard, then the construction from \autoref{fig:hvcinst} is an individually correct, $(\rho,\tern_\alpha,\epsilon)$-probabilitically homomorphic, robustly homomorphic, and position binding HVC for domain $\ring^{\xi}_{q'}$ and vector length $2^\tau$.
\end{theorem}
\begin{proof}
...
\begin{lemma}\label{lem:hvcposbind}
  Let $n,q,q',\alpha_w,\rho,\eta,\tau,\xi,\bagg,\epsilon$ be positive integers such that $n$ is a power of two, $q,q'$ are prime.
  Let $\ring_q,\ring_{q'}$ be the polynomial rings $\ZZ_q[x]/\langle x^n+1\rangle$ and $\ZZ_{q'}[x]/\langle x^n+1\rangle$ respectively.
  If the $\sis_{\ring,q,2\lceil \log_{2\eta+1} q \rceil,2\bagg}$ problem and the $\sis_{\ring,q,\xi\lceil \log_{2\eta+1} q' \rceil,2\bagg}$ problem are hard, then the construction from \autoref{fig:hvcinst} is position binding.
\end{lemma}
\begin{proof}
We will prove this lemma by leveraging that any pair of valid decommitments for different messages will lead to a collision somewhere in the generalized hash tree, which can be turned into a solution for one of the SIS instances. 
  ...
  \[
    \Pr[]
  \]
  We construct a PPT algorithm that solves the $\sis_{\ring,q,2\lceil\log q\rceil,4\rho\alpha}$ problem as follows.
  Upon input $\vec{a}=(a_0,\dots,a_{2\lceil\log q\rceil-1})^\intercal$, $\bdv$ sets $\vec{h}_0:=(a_0,\dots,a_{\lceil\log q\rceil-1})^\intercal$ and $\vec{h}_1 := (a_{\lceil\log q\rceil},\dots,a_{2\lceil\log q\rceil-1})^\intercal$ and runs $(\vec{c},t,\vec{d}^0,\vec{d}^1)\gets \adv((\vec{h}_0,\vec{h}_1))$.
  For $i\in\bin$ let $m^i := \wverify((\vec{h}_0,\vec{h}_1),\vec{c},t,\vec{d}^i)$.
  If $m^0 = m^1$ or $\bot\in\{m^0,m^1\}$, $\bdv$ aborts.
  Otherwise, parse $\vec{d}^i$ as $(\vec{p}^i_1,\dots,\vec{p}^i_{\tau},\vec{s}^i_1,\dots,\vec{s}^i_\tau)$, set $\vec{p}^i_0 := \vec{c}$.
  
  Let $j^*\in [\tau]$ be the \emph{largest} index, such that $\proj(\vec{p}^0_{j^*})=\proj(\vec{p}^1_{j^*})$.
  Note that such an index always exists, since $\vec{p}^0_0 = \vec{c} = \vec{p}^1_0$ and that $j^*< \tau$, since $\proj(\vec{p}^0_{j^*})=m^0\neq m^1=\proj(\vec{p}^1_{j^*})$.
  If $\tilde t_{j^*} = 0$, $\bdv$ outputs $
  \vec{z}:=(\vec{p}_{j^*+1}^0, \vec{s}_{j^*+1}^0)^\intercal - (\vec{p}_{j^*+1}^1, \vec{s}_{j^*+1}^1)^\intercal$, if $\tilde t_{j^*} = 1$, $\bdv$ outputs $\vec{z}:=(\vec{s}_{j^*+1}^0, \vec{p}_{j^*+1}^0)^\intercal - (\vec{s}_{j^*+1}^1, \vec{p}_{j^*+1}^1)^\intercal$.
  
  We now analyze the success probability of $\bdv$.
  It holds that $\proj(\vec{p}_{j^*}^0) = \proj(\vec{p}_{j^*}^1)$ and by the definition of the weak verification algorithm that
  \begin{align*}
    &\vec{h}_{\tilde t_{j^*}}^\intercal\cdot\vec{p}_{j^*+1}^0 + \vec{h}_{\tilde t_{j^*}\xor 1}^\intercal\cdot\vec{s}_{j^*+1}^0 = \vec{h}_{\tilde t_{j^*}}^\intercal\cdot\vec{p}_{j^*+1}^1 + \vec{h}_{\tilde t_{j^*}\xor 1}^\intercal\cdot\vec{s}_{j^*+1}^1\\
    \iff&\vec{h}_{\tilde t_{j^*}}^\intercal\cdot(\vec{p}_{j^*+1}^0-\vec{p}_{j^*+1}^1) + \vec{h}_{\tilde t_{j^*}\xor 1}^\intercal\cdot(\vec{s}_{j^*+1}^0-\vec{s}_{j^*+1}^1) = 0\\
    \iff&\vec{a}^\intercal\cdot\vec{z}=\vec{0} 
  \end{align*}
  It further holds by the definition of the weak verification algorithm that 
  \[\norm{\vec{p}_{j^*+1}^0} \leq 2\rho\alpha,\quad \norm{\vec{s}_{j^*+1}^0} \leq 2\rho\alpha, \quad\norm{\vec{p}_{j^*+1}^1} \leq 2\rho\alpha, \quad \norm{\vec{s}_{j^*+1}^1} \leq 2\rho\alpha.\]
  Therefore, the norm of $\vec{z}$ can be bounded as \[\norm{\vec{z}} \leq \max\{\norm{\vec{p}_{j^*+1}^0},\norm{\vec{s}_{j^*+1}^0}\}+\max\{\norm{\vec{p}_{j^*+1}^1},\norm{\vec{s}_{j^*+1}^1}\} \leq 4\rho\alpha.\]
  It remains to show that $\vec{z}\neq 0$.
  Since $j^*$ is the \emph{largest} index such that 
  \[
    \proj(\vec{p}_{j^*}^0) = \proj(\vec{p}_{j^*}^1)
  \]
  it holds that
  \[
    \proj(\vec{p}_{j^*+1}^0) \neq \proj(\vec{p}_{j^*+1}^1)
  \]
  and thereby that
  \[
    \vec{p}_{j^*+1}^0 \neq \vec{p}_{j^*+1}^1.
  ...
  \begin{align*}
  \negl \geq& \Pr[
      \vec{a} \gets \ring_q^{2\lceil\log q\rceil}; \vec{s}\gets\bdv(\vec{a}) : \vec{z}\in\ball_{4\rho\alpha}^{2\lceil\log q\rceil}\setminus\{\vec{0}\} \land \vec{a}^\intercal\vec{s}=0
    ]\\
    =&\Pr\mleft[\begin{aligned}
      \params\gets&\mspace{\medmuskip}\setup(\secparam,\tau);\\
      (c,t,d_1,d_2) \gets&\mspace{\medmuskip} \adv(\params);\\
      m_1 \gets&\mspace{\medmuskip} \wverify(\params,c,t,d_1);\\
      m_2 \gets&\mspace{\medmuskip} \wverify(\params,c,t,d_2)      \end{aligned}: \begin{aligned}
      &\mspace{\medmuskip}m_1\neq m_2 \\\land&\mspace{\medmuskip}\bot\not\in\{m_1,m_2\}
      \end{aligned}
    \mright]\tag*{\raisebox{-1cm}{\qed}}
  \end{align*}
\end{proof}
