...
impl PartialEq for HVCPoly {
    fn eq(&self, rhs: &HVCPoly) -> bool {
        self.coeffs
            .iter()
            .zip(rhs.coeffs.iter())
            .map(|(&x, &y)| (x - y) % HVC_MODULUS as i32 == 0)
            .fold(true, |acc, x| acc & x)
    }
}

impl Default for HVCPoly {
    fn default() -> Self {
        Self {
...
impl AddAssign for HVCPoly {
    // Coefficient wise additions with mod reduction.
    fn add_assign(&mut self, other: Self) {
        self.coeffs
            .iter_mut()
            .zip(other.coeffs)
            .for_each(|(x, y)| *x = (*x + y) % HVC_MODULUS as i32)
    }
}
...
impl HVCPoly {
    ...
    pub(crate) fn schoolbook(a: &Self, b: &Self) -> Self {
        ...
        for i in 0..N as usize {
            for j in 0..N as usize {
                buf[i + j] +=
                    ((a.coeffs[i] as i64) * (b.coeffs[j] as i64) % HVC_MODULUS as i64) as i32;
            }
        }
        for i in 0..N as usize {
            c[i] = crate::poly::lift(buf[i] - buf[i + N as usize], HVC_MODULUS) as i32;
        }
        Self { coeffs: c }
    }
    ...
    pub fn rand_poly<R: Rng>(rng: &mut R) -> Self {
        ...
        for e in res.coeffs.iter_mut() {
            ...
            while tmp >= HVC_SAMPLE_THRESHOLD {
                tmp = rng.next_u32();
            }
            *e = (tmp % HVC_MODULUS as u32) as i32 - HVC_MODULUS_OVER_TWO
        }
        res
    }
    ...
    pub fn decompose(&self) -> [HVCPoly; HVC_WIDTH] {
        ...
        let mut base_coeffs: Vec<_> = self.coeffs.iter().map(|&x| normalize(x, HVC_MODULUS)).collect();
...
mod test {
    ...
    use crate::{HVCPoly, HVC_MODULUS};

    #[test]
    fn test_normalization() {
        let mut rng = ChaCha20Rng::from_seed([0u8; 32]);
        for _ in 0..10 {
            let mut poly = HVCPoly::rand_poly(&mut rng);
            ...
            poly2
                .coeffs
                .iter_mut()
                .for_each(|x| *x = *x + ((rng.next_u32() % 100) * HVC_MODULUS as u32) as i32);
            poly2.lift();
