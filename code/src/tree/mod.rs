...
pub struct Tree {
    ...
    non_leaf_nodes: Vec<HVCPoly>,

    /// store the hash of leaf nodes from left to right
    leaf_nodes: Vec<HVCPoly>,
}
...
impl Tree {
    ...
    pub fn init(hasher: &HVCHash) -> Self {
        let leaf_nodes = vec![HVCPoly::default(); 1 << (HEIGHT - 1)];
    ...
    pub fn new_with_leaf_nodes(leaf_nodes: &[HVCPoly], hasher: &HVCHash) -> Self {
        ...
        let mut non_leaf_nodes = vec![HVCPoly::default(); (1 << (HEIGHT - 1)) - 1];
...
mod test {
    ...
    fn test_tree() {
        ...
        let leafs: Vec<HVCPoly> = (0..(1 << (HEIGHT - 1)))
